import { Direction, Order } from "../../generated/prisma/client";
...
await prisma.$transaction(async (tx) => {
  ...
  const account = await tx.$queryRaw<{ id: string; balance: number; marginUsed: number }[]>`
  ...
  const requiredMargin = calcRequiredMargin(order.quantity, currentPrice, order.leverage)
  ...
  if (requiredMargin > freeMargin) {
    ...
    console.log(`Order ${order.id} cancelled — insufficient margin. Required: ${requiredMargin}, Free: ${freeMargin}`)
    return
  }
  ...
  await tx.account.update({
    where: { id: order.accountId },
    data: { marginUsed: { increment: requiredMargin } },
  })
  ...
  const position = await tx.position.findFirst({
    where: { accountId: order.accountId, symbol: order.symbol, isOpen: true },
  })
